...
![Dashboard Screenshot](dashboard-v1.1.0.png)
...
Always-visible metrics providing a quick snapshot of your Claude Code usage:

**Cost & Efficiency Metrics:**
- **Monthly Cost** - Total spending over the last 30 days (fixed 30-day window)
- **Monthly Tokens** - Total token usage over the last 30 days (fixed 30-day window)
- **Cost (Selected)** - Total cost within the selected time range
- **Cost / Hour** - Average hourly spending rate
- **Cost per 1M Tokens** - Cost efficiency metric (dollars per million tokens)
...
- **Net Lines** - Net change in lines of code (added - removed)
- **Lines per Dollar** - Code productivity efficiency metric

### Sessions & Activity (Collapsible Section)
Session and activity metrics consolidated in one place:

- **Sessions** - Total number of active sessions in the selected time range
- **Active Users** - Number of unique users
- **Avg Session Length** - Average duration of sessions
- **Active Time** - Total active time across all sessions
- **Cost per Active Hour** - Cost efficiency per hour of active work

### Trends (Collapsible Section)
Rate-based metrics for tracking usage patterns over time:

- **Cost Rate ($/hour)** - Spending rate trend over time by model
- **Token Rate (per hour)** - Token usage rate trend over time by type

### Cost Analysis (Collapsible Section)
Detailed cost breakdown with sortable tables:

- **Cost by Model** - Table showing spending by model (Opus, Sonnet, Haiku), sorted highest to lowest
- **Tokens by Model** - Table showing token usage by model, sorted highest to lowest
- **Cost by User** - Table showing per-user spending breakdown

### Token & Efficiency (Collapsible Section)
Token usage metrics and cache efficiency monitoring:

- **Cache Hit Rate** - Percentage of tokens served from cache (target: >80% for optimal cost savings)
- **Input Tokens** - Total tokens sent to the API
- **Output Tokens** - Total tokens generated by Claude
- **Cache Read** - Tokens served from cache (reduces costs significantly)
- **Cache Creation** - Tokens used to create new cache entries

All panels respond to Grafana's time picker (except 30-day fixed totals like "Monthly Cost" and "Monthly Tokens"), allowing you to analyze:
- Last 1 hour
- Last 6 hours
- Last 24 hours
- Last 7 days
...
The following metrics are included in the dashboard but have **known limitations**:
...
**Note:** These metrics provide directional insights but should not be relied upon for precise productivity tracking.

### Metrics Not Available

The following metrics are **not emitted** by Claude Code:

| Metric | Status |
|--------|--------|
| `claude_code_pull_request_count_total` | Not emitted |
